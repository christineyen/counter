...
def get_cumu_logs_for_set(buddy_sns=None):
    ...
    if buddy_sns:
      for buddy_sn in buddy_sns:
          ...
          cur.execute(GET_BUDDY_LOG_ENTRIES_SINGLE_QUERY, (user_id, buddy_id))
          entries = __get_cumu_logs(cur, user_id, buddy_id, buddy_sn)
          all_entries.append(entries)
    else:
      cur.execute(GET_BUDDY_LOG_ENTRIES_ALL_QUERY, (user_id,))
      all_entries.append(__get_cumu_logs(cur, user_id))

    util.close_connection()
    return all_entries


def __get_cumu_logs(cur, user_id, buddy_id=0, buddy_sn=''):
